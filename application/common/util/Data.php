...
final class Data
{
    ...
    static public function channelLevel($data, $pid = 0, $html = "&nbsp;", $fieldPri = 'cid', $fieldPid = 'pid', $level = 1)
    {
        ...
        foreach ($data as $k => $v) {
            if ($v[$fieldPid] == $pid) {
                $arr[$i] = $v;
                $arr[$i]['_level'] = $level;
                $arr[$i]['_html'] = str_repeat($html, $level - 1);
                $arr[$i]["_data"] = self::channelLevel($data, $v[$fieldPri], $html, $fieldPri, $fieldPid, $level + 1);
                $i += 1;
            }
        }
        return $arr;
    ...
    static public function tree($data, $title, $fieldPri = 'cid', $fieldPid = 'pid')
    {
        if (!is_array($data) || empty($data))
            return array();
        $data = self::channelLevel($data, 0, '&nbsp;', $fieldPri, $fieldPid, 1);
        $data = self::changeChanelLevel($data, [], 0);
        foreach ($data as $key => $value) {
            if ($value['_level'] == 1) {
                $data[$key]['_name'] = '&emsp;' . $value[$title];
                continue;
            }
            $str = '';
            for ($i = 1; $i < $value['_level'] - 1; $i++) {
                if ($value['_parent_end'] != 1) {
                    $str .= "&emsp;│";
                } else {
                    $str .= '&emsp;&emsp;';
                }
            ...
            $data[$key]['_name'] = $str . '&nbsp;' . $value[$title];
        }
        return $data;
    }

    static public function changeChanelLevel($data, $result = [], $parentIsEnd = 0)
    {
        if (!is_array($data) || empty($data)) {
            return $result;
        }
        foreach ($data as $key => $value) {
            $_end = isset($data[$key + 1]) ? 0 : 1;
            $value['_end'] = $_end;
            $value['_parent_end'] = $parentIsEnd;
            $_data = $value['_data'];
            unset($value['_data']);
            $value['_has_child'] = $_data ? 1 : 0;
            array_push($result, $value);
            if ($_data) {
                $result = self::changeChanelLevel($_data, $result, $_end);
            }
        }
        return $result;
    }
